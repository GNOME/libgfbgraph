...
{
        g_autoptr (GFBGraphAlbum) val = gfbgraph_album_new ();
...
{
        g_autoptr (GFBGraphNode) val = gfbgraph_node_new ();
        g_assert_nonnull (val);
}
...
{
        g_autoptr (GFBGraphPhoto) val = gfbgraph_photo_new ();
        g_assert_nonnull (val);
}
...
{
        g_autoptr (GFBGraphUser) val = gfbgraph_user_new ();
        g_assert_nonnull (val);
}
